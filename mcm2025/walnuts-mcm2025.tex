...
\item \myemph{Biased-progressive} NUTS samples \myemph{as above}, favoring 1/4 orbits.
\end{itemize}
...
\item If $\theta \sim \textrm{normal}(\mu, \Sigma)$, then $M = \Sigma^{-1}$ is a \myemph{perfect preconditioner}
  ...
  \end{subitemize}
\item NUTS uses \myemph{fixed warmup iterations} before smapling to estimate $\textrm{cov}[\Theta]$
...
\item By adjusting step size over \myemph{4 orders} of magnitude, \myemph{WALNUTS succeeds}\\[12pt]
...
\item \myemph{Neal's funnel}: condition varies 0 to 1000+, eigenstructure varies
\end{itemize}

\sld{Gibbs self tuning (GIST)}
\begin{itemize}
\item On the road to WALNUTS, we came up with \myemph{Gibbs self tuning}
...
\item While selecting steps $L$ w.\ NUTS doesn't preserve Hamiltonian within $\tau$ between any two states on trajectory
  ...
  \item $\epsilon = \epsilon / 2$
    \end{subitemize}
\item \myemph{Hastings ratio} 1 if same number of halvings forward and backward; else reject.
\end{itemize}

\sld{WALNUTS}
\begin{itemize}
\item Just like GIST II, but \myemph{every leapfrog step} \\[12pt]
  \includegraphics[width=0.6\textwidth]{img/funnel_hist.pdf}
\item \myemph{Et voila!}
\end{itemize}

\sld{Adaptation}
\begin{itemize}
\item Code repro uses a continuous, diagonal form of \myemph{Nutpie's mass matrix} adaptation scheme (Seyboldt et al. in prep)
  \begin{subitemize}
  \item Minimizes Fisher divergence (of log norm of gradients of log density, aka scores)
  \item uses \myemph{covariance of draws}, like NUTS, and
  \item \myemph{covariance of scores}, which is inverse covariance in multi-normal
  \item \myemph{Initialize} at absolute gradient of log density (regularized outer product estimate)
  \end{subitemize}
\item Use \myemph{dual averaging} a la NUTS for setting $\epsilon^\textrm{max}$ to target optimal 2/3 accept.
\end{itemize}

\sld{Next steps}
\begin{itemize}
\item Local mass matrix adaptation.
  \begin{subitemize}
  ...
  \item perhaps use \myemph{low rank plus diagona} to reduce from $\mathcal{O}(N^3)$ to $\mathcal{O}(K^2 N)$.
    \end{subitemize}
\item Local max step size estimation.
\vfill
\item We're open to suggestions.
\end{itemize}
