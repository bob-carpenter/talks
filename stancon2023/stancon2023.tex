...
\item No-U-turn sampler (NUTS) automatically tunes
  ...
  \end{subitemize}
\item Betancourt (2017) revised NUTS for Stan
\end{itemize}
...
\item Each \myemph{retry cuts step size} by constant $c$ and \myemph{multiplies steps}
  ...
  \end{subitemize}
\begin{center}
  \spc\includegraphics[height=1.1in]{img/dr.png} \hfill (HMC wall vs. accept)
\end{center}
\end{itemize}
...
\begin{itemize}
\item lazy evaluation plus static compilation 
  \begin{subitemize}
  \item how C++ can be so much faster than C for matrices 
  \end{subitemize}
  \item original Stan \myemph{evaluated} expression templates 
  \item improved Stan tries to \myemph{propagate} them 
