...
\item \myemph{Input}: initial position $\draw{\theta}{0}$, step size
  $\epsilon$, steps $L$, metric $\Sigma$
\item For each iteration $m$
  \begin{subitemize}
  \item (Gibbs) Resample momentum $\rho \sim \textrm{normal}(0, \Sigma)$
  \item (Metropolis) Run leapfrog algorithm $L$ steps from $(\draw{\theta}{m-1}, \rho)$
    to $(\theta^*, \rho^*)$
  \item if $\textrm{uniform}(0, 1)
    < \textrm{min}\left(1, \frac{\displaystyle \strut \exp(-H(\theta^*, \rho^*))}
           {\displaystyle \strut \exp(-H(\draw{\theta}{t-1}, \rho))}\right)$, then
           $(\draw{\theta}{t}, \draw{\rho}{t}) = (\theta^*, -\rho^*)$
  \item else $(\theta_t, \rho_t) = (\draw{\theta}{t-1}, \draw{\rho}{t-1})$.
  \end{subitemize}
\item \myemph{Output}: sample $\draw{\theta}{1}, \ldots, \draw{\theta}{M}$
  \vfill
\item \myemph{Generalized HMC}: Partially resample $\rho =
  \sqrt{\lambda^2} \cdot \draw{\rho}{t-1} + \sqrt{1 - \lambda^2} \cdot
