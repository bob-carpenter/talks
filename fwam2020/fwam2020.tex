...
\mbox{ }
\\[12pt]
\spc{{\LARGE\bfseries \color{MidnightBlue}{Probabilistic Programming}}}
\\[4pt]
\spc{\large {and Bayesian Inference with Stan}}
\\[36pt]
\noindent
...
\sld{Simulated Binary Data}
%
\begin{itemize}
\item True .300 batting average (BA); simulate 10 at bats (AB)
\begin{subitemize}
\item {\bfseries 10 at bats} \hfill (.000 to .600 BA)
\\[4pt] 1 1 2 6 2 0 4 4 4 2
\vspace*{3pt}
%
\item {\bfseries 100 at bats} \hfill (0.220 to 0.330 BA)
\\[4pt] 22 34 30 26 25 31 31 28 27 33
\vspace*{3pt}
%
\item {\bfseries 1000 at bats} \hfill (.294 to .320 BA)
\\[4pt] 296 307 302 298 290 297 303 294 280 320
\vspace*{3pt}
%
\item {\bfseries 10,000 at bats} \hfill (.295 to .305 BA)
...
\begin{subitemize}
\item \myemph{test model} and sofware on simulated data (\myemph{does
    it work?}),
\item perform prior predictive checks (\myemph{is prior sensible?})
\item \myemph{fit model} to real data,
\item perform posterior predictive checks (\myemph{does it fit
    data?}), and
\item perform held-out checks/cross-validation (\myemph{does it predict?}).
\end{subitemize}
\end{enumerate}
...
% \sld{Models for counts}
% \begin{itemize}
% \item Stan and emcee for Poisson regression in LIGO:
% \vspace*{-6pt}
% \begin{center}
% \includegraphics[width=0.75\textwidth]{img/ligo-stan.png}
% \end{center}
% \end{itemize}
...
\item AI is not about a technique
  \begin{subitemize}
  \item according to Google, regression is AI!
  \end{subitemize}
\item ML is more difficult to characterize
\end{itemize}

\sld{ML and Stats (1)}
\begin{itemize}
\item Traditional stats is about data analysis
\item ML is about prediction
  \begin{subitemize}
  \item this is why ML is eating stats' lunch
  \end{subitemize}
\item Stats is about calibrated uncertainty quantification
  \begin{subitemize}
  \item for estimators, traditional hypothesis testing
  \item for predictions, Bayesian posteriors
  \end{subitemize}
\item Bayesian stats is about prediction \textsl{and} data analysis
\end{itemize}
...
\item Inference
  \begin{subitemize}
  \item ML focuses on prediction based on point estimates
  \item traditional stats focuses on estimation and testing
  \item Bayesian stats on posterior uncertainty quantification 
  \end{subitemize}
\end{itemize}
...
\item Scale
  \begin{subitemize}
  \item ML only works well with big data
  \item Stats tries to squeeze info out of small data
  \item Stats builds more model with more data (spatio-temporal,
    sub-population, etc.)
  \end{subitemize}
\end{itemize}
  
\sld{TensorFlow or PyTorch or JAX?}
\begin{itemize}
...
\item Focus of all of these libraries is
  \begin{subitemize}
  \item approximate algorithms and
  \item neural networks.
  \end{subitemize}
\item Biggest difference is static vs.\ dynamic autodiff
\begin{subitemize}
  \item Pytorch/JAX are dynamic like Stan
  \item TensorFlow optimizes static autodiff
  \item Theano (PyMC3) symbolic
\end{subitemize}
\end{itemize}
...
\item embedded in Python
  \begin{subitemize}
    \item variables are composable Python objects
    \item NumPy punned inside PyMC3 namespace
  \end{subitemize}
