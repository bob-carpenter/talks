...
* Stan code is translated to a C++ model class
* C++ model class relies on and includes the Stan math library
* C++ model class then compiled by C++ compiler, along with C++ algorithms and math library
* Result is an executable (CmdStan) or dynamically linked object code (RStan, PyStan)

## Stan Model Class
* Known interface with a variety of methods the interfaces rely on
* Constructed with the data you pass in when fitting the model
...
* Also written in C++
* Lives in the Stan repo (`lang`) alongside the algorithms and services
...
* Math library directly includes some external libraries such as Boost, Eigen, Sundials at `lib/`
