...
## Stan language is translated to a C++ class
* Creates a C++ model class with a known concept (C++ interface requirement)
* Model implements a `log_prob` function over *data and constrained parameters*
    - log posterior as function of data up to constant (usually joint via Bayes's rule)
    - or log penalized likelihood for maximum likelihood applications
    - applies to vector of unconstrained parameters
    - only required up to proportion (additive constant on log scale)
* Model class is constructed with the data
    - resulting function defines log posterior (or penalized likelihood) up to constant
* Also transforms parameters from unconstrained to constrained and back
    - with automatic log Jacobian corrections
    - algorithms work with unconstrained parameters
...
* In `stan/lang` directory of the Stan repository
   - with I/O, inference algorithms, service and memory management code
