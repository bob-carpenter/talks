...
\item Step size adaptive NUTS is \myemph{suboptimal} in that it
  ...
  \item but \myemph{trajectories vary} in \myemph{scale/curvature} (e.g., flat mouth, curved neck)
  \end{subitemize}
...
\item Use \myemph{delayed rejection} with Hamiltonian Monte Carlo
  ...
  \end{subitemize}
\item \myemph{Improves delayed rejection} by only retrying if probabilty of \myemph{acceptance is low} (additional balance).
\item \myemph{Works} for funnel.
\item We haven't tried \myemph{DR-NUTS}.
\item {\small Chirag Modi, Alex Barnett, Bob Carpenter. 2024. \myemph{Delayed rejection Hamiltonian Monte Carlo for sampling multiscale distributions}. \textit{Bayesian Analysis} 19(3). {\small arXiv-ed 2021}.}
\end{itemize}
...
\item {\small Gilad Turok, Chirag Modi, Bob Carpenter. 2024. \myemph{Sampling from multiscale densities with delayed rejection generalized HMC}. \textit{arXiv}:2406.02741}
\end{itemize}
