...
\item {\small Bou-Rabee, N., Carpenter, B., Kleppe, T.S. and Liu, S., 2026. \myemph{The Within-Orbit Adaptive Leapfrog No-U-Turn Sampler}. \textit{JMLR}}.
\end{itemize}
...
\begin{itemize}
\item WALNUTS needs: \myemph{mass matrix} and \myemph{macro step size}
...
\item Fisher divergence minimizes \myemph{gradient discrepancy},
  $$
  \textrm{D}_F[q \, || \, p] =
  \int_{\mathbb{R}^D} q(\theta) \cdot || \nabla \log q(\theta) - \nabla \log
  p(\theta)||^2 \ \textrm{d}\theta.
  $$
\end{itemize}
...
\item For normal, $\mathbb{E}[S(\Theta) \cdot S(\Theta)^\top] =
