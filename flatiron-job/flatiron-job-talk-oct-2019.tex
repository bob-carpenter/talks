...
\item but \myemph{bad statistics}.
\end{subitemize}
\end{itemize}
...
\end{subitemize}
\item Narrowly concentrated predictions are sharper.
\begin{subitemize}
\item $\mbox{Pr}[\alpha \in (.58, .59)] = .9$ sharper than
$\mbox{Pr}[\alpha \in (.5, .6)] = .9$
\end{subitemize}
\item Probabilistic predictions nearer 0 and 1 are sharper
\begin{subitemize}
...
\\[-6pt]
\item \myemph{Goal}: consistent, useful inference from finite data.
\begin{subitemize}
...
$
\displaystyle
\textrm{sd}\!\left[\textrm{util}(a, \theta) \mid y\right]
\ = \ \sqrt{\textrm{var}\!\left[\textrm{util}(a, \theta) \mid y\right]}
$
\\[4pt]
$
\displaystyle
\textrm{var}\!\left[\textrm{util}(a, \theta) \mid y\right]
\ = \
\int_{\Theta} (\textrm{util}(a, \theta) - \mu)^2 \cdot p(\theta \mid y) \ \textrm{d}\theta.
$
\end{itemize}
...
$$
\myemph{subject to risk threshold} $\sigma$,
$$
\textrm{sd}\!\left[\textrm{util}(a, \theta) \mid y\right] < \sigma.
$$
\item \myemph{Efficient frontier}
\vspace*{-16pt}
\begin{center}
\mbox{ } \hfill \hfill \hfill \hfill
\includegraphics[width=0.5\textwidth]{img/efficient-frontier.pdf}
\hfill \mbox{ }
\end{center}
\end{itemize}

\sld{Examples of decision making}
\begin{itemize}
\item Example of \myemph{differeing risks} with \myemph{same expected utility}:
\begin{subitemize}
\item 50\% chance of nothing, 50\% chance of \$1M
\item 50\% chance of \$550K, 50% chance of \$450K
\end{subitemize}
\item \myemph{Online A/B testing}: choose next action with highest probability of being best.
\end{itemize}
...
\sld{Area under the R.O.C.\ curve}
\begin{center}
...
\end{subitemize}
\item Time spent on evaluation and visualization
...
\begin{subitemize}
\item with approximate variational inference
\item MCMC more accurate but slower (not evaluated)
\end{subitemize}
\end{itemize}
...
\begin{itemize}
\item marketing attribution at Google, Domino's Pizza, Legendary Ent., $\ldots$
\item demand forecasting at Facebook, Salesforce, $\ldots$
\item time series models at Two Sigma, Point72, $\ldots$
\item pharmacology and Phase I CTs at Novartis, Pfizer, Astra Zeneca, $\ldots$
\item (e-)sports analytics at Tampa Bay Rays, NBA, Sony Playstation, $\ldots$
\item $\ldots$
\end{itemize}
