...
\item Plus a preview of my \myemph{forthcoming work} on
\begin{subitemize}
\item algorithms,
...
\\[-6pt]
\item Goal: consistent, useful inference from finite data.
\begin{subitemize}
...
\item Dramatically \myemph{outperforms} the state of the art:
\begin{quote}
...
\item No magic---we just \myemph{turn the Bayesian crank} using Stan
\end{itemize}
...
\begin{itemize}
\item Consider multiple genes (5000+ differentially splice in humans)
\item Consider \myemph{different groups} of cells
\begin{subitemize}
\item e.g., normal liver vs. tumor; or liver vs. kidney
\end{subitemize}
\item Consider \myemph{multiple subjects} in each group
\begin{subitemize}
...
\item \myemph{variation} within and between groups.
\end{subitemize}
\end{itemize}
...
\item Replicate inclusion count (observed)
\begin{subitemize}
\item $y_{i, j, k} \sim \textrm{binomial}(N_{i,j,k}, \ \textrm{logit}^{-1}(\psi_{i, j, k}))$
\end{subitemize}
\item Replicate inclusion log odds
\begin{subitemize}
\item $\psi_{i,j,k} \sim \textrm{normal}(\phi_{i,j}, \ \sigma_{i,j})$
\end{subitemize}
\item Group inclusion log odds
\begin{subitemize}
...
\phi_{i,2} = \theta_i - \nu_i$
\end{subitemize}
\item Gene inclusion log odds
\begin{subitemize}
\item $\theta_i \sim \textrm{normal}(0, \gamma)$
\end{subitemize}
\end{itemize}
