...
\item Calibrated predictions of the {\slshape same event}
\begin{subitemize}
\item Will it \myemph{rain in Edinburgh} on any given day?  (52\%)
\item $\ldots$ on any given day in December?  (65\%)
\item $\ldots$ tomorrow?  (80\%)
\end{subitemize}
\end{itemize}
...
\item So, need to test fit real data.
\end{subitemize}
\end{itemize}
...
\item \myemph{Computational goal}: keep extending efficient frontier
\begin{subitemize}
\item already pushed $10^2{-}\infty$ efficiency and scalability
\end{subitemize}
% \begin{center}
...
next action $a_{t+1}$ chosen w.\ probability proportional to $k$ being best
\begin{subitemize}
\item aka Thompson sampling
\end{subitemize}
...
\vspace*{-6pt}
\item \footnotesize {\slshape sensitivity} = true positive rate; 
\ \ {\slshape specificity} = 1 - false positive rate
\end{subitemize}

\sld{All with minimal effort}
\begin{itemize}
\item one grad student over three months part time; mostly eval
\item uses default variational inference
\item Stan program for model is half a page long
\begin{subitemize}
\item hierarchical modeling automatically pools
\item i.e., shares strength, regularizes, shrinks effects
\end{subitemize}
...
\sld{Goldilocks Adaptation {\large (prototyped)}}
\vspace*{-6pt}
\begin{center}
...
\vspace*{-12pt}
\begin{itemize}
\item find \myemph{typical set} ($\theta s.t.\ \textrm{H}[\theta] \pm \epsilon$) \myemph{in parallel}
...
\begin{subitemize}
\item (too cold) points in tail trend upward in log density
\item (too hot) points near mode trend downard
\item (just right) those in typical set bounce around
\end{subitemize}
\end{itemize}
...
\item \myemph{Automatic differentiation} (higher-order; matrix; testing)
...
\begin{subitemize}
\item catching up to Stan functionality with growing staffing
\end{subitemize}
