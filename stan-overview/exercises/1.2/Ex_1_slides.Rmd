...
knitr::include_graphics("m8ball-patent.png")
```
...
## RStan workflow for this set of exercises
...
The `print` provides information about the sample and the controls used to generate it.
...
* Generalize program `gen_d6.stan` to `gen_sum_N_rolls_dK.stan` so that it outputs the sum of N rolls of any platonic solid with number of faces K.
    + calculate the expectation of the sum of 4 rolls of a D6
    + calculate the expectation of the sum of 2 rolls of a D12
    + use R function `hist` to visualize the resulting distribution of sums - compare and constrast

## Exercise 4: More games

* Write a Stan program `gen_sum_best_3_of_4_d6.stan` which generates the sum of the best 3 out of 4 rolls of a fair, 6-sided die.

* Compare `gen_sum_best_3_of_4_d6.stan` to the result of running `gen_sum_N_rolls_dK.stan` for 3 rolls of a 6-sided die.
    + Use RStan function `stan` to generate a sample consisting of 4000 draws for each model
    + Get the vector of all 4000 outcomes
    + use R function `hist` to visualize the resulting distribution of sums - compare and constrast
    + use R function `hist` to visualize the resulting distribution of sums.
