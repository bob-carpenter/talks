...
```
library(ggplot2);
rolls = as.data.frame(fit_8ball, pars=c("ans"))
p2 = ggplot(data=rolls, aes(rolls$ans)) +
...
Compare this to visualizations produced using R's base graphics package:
```
# visualize in R
> hist(a_rolls, xlab="outcome", ylab="frequency")
> plot(table(a_rolls), xlab="outcome", ylab="frequency")
```
