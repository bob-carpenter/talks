...
\sld{\Large What does Stan Compute?}
%
\begin{subitemize}
%
...
\theta^{(1)}, \ldots, \theta^{(M)}
\]
\item Posterior \myemph{parameter estimates} as means
\[
\hat{\theta}
\ = \
\mathbb{E}[\theta \mid y]
...
\]
%
\item Posterior \myemph{parameter variance estimates} as variances
\[
...
\sld{\Large What does Stan Compute? (cont.)}
%
\begin{subitemize}
\item Posterior \myemph{event probabilities} as expectations of indicators
\[
...
\]
%
\item Posterior \myemph{predictions} by marginalizing parameters
\[
p(\tilde{y} | y)
 =
\mathbb{E}[p(\tilde{y}) | y]
...
\]
%
\item Posterior \myemph{quantiles} by sample quantiles (e.g.,
posterior \myemph{intervals} and posterior \myemph{median} estimates)
\[
...
\item Stan is an \myemph{imperative probabilistic programming language}
  \begin{subitemize}
  \item  cf., BUGS: declarative; \ Church: functional; \ PyMC3: ?
  \end{subitemize}
...
\end{subitemize}
\item \myemph{Ongoing open source development}
...
{\small
\[
\frac{\mbox{probability of being at } \theta^*
