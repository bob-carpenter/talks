...
\includegraphics[width=\textwidth]{img/ball-in-cube.pdf}

{\footnotesize\slshape 4000 simulations; those in unit ball shown in blue.}
\end{minipage}
...
\end{stancode}
\begin{subitemize}
\item \code{in\_ball} is value of indicator function.
\item indicator function implicit in comparison (\code{<=}).
\item Posterior mean of \code{in\_ball} estimates
$\mbox{Pr}[x^2 + y^2 + z^2 \leq 1]$.
\end{subitemize}

\sld{Ball in Cube in R}
%
\begin{itemize}
\item To run in R, use \code{Fixed\_param} option.

\begin{stancode}
> fit <- stan("ball-in-cube.stan", algorithm="Fixed_param",
              iter=10000)
> print(fit, probs=c(), digits=3)

        mean se_mean  sd n_eff Rhat
in_ball 0.52   0.004 0.5 19780    1
\end{stancode}
\item 0.52 is our estimate of $\mbox{Pr}[x^2 + y^2 + z^2 \leq 1]$
\item The standard error of 0.004 indicates the 95\% interval is
  roughly $0.52 \pm 0.008$
\end{itemize}
