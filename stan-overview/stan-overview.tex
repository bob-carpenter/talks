...
%
\noindent 
\spc{\LARGE\bfseries \color{MidnightBlue}{Stan Overview}}
\\[4pt]
\noindent 
\spc{\slshape Stan Development Team {\small (in order of joining)}:}
\\[2pt]
\spc{\footnotesize
Andrew Gelman,
\  \myemph{Bob Carpenter},
\ \myemph{Daniel Lee},
\ \myemph{Ben Goodrich},
\ Michael
\\ \spc
...
\ Rob Trangucci,
\ Rob Goedman,
\ \myemph{Jonah Sol Gabry},
\\ \spc
\myemph{Robert L.\ Grant},
\ Brian Lau,
\ Krzysztof Sakrejda,
\ \myemph{Aki Vehtari},
\ Rayleigh
...
\myemph{Sean Talts},
\ \myemph{Ben Bales},
\ \myemph{Ari Hartikainen},
\ \myemph{Matthijs V\'ak\'ar},
\\ \spc
Andrew Johnson,
\ \myemph{Dan Simpson},
\ \myemph{Yi Zhang},
\ \myemph{Paul B\"urkner},
\ \myemph{Steve Bronder},
\\ \spc
\myemph{Rok Cesnovar},
\ \myemph{Erik Strumbelj},
\ Edward Roualdes
}
\\[4pt]
\spc{\footnotesize\slshape 34 active devs, \ $\approx 10$ full-time equivalents}
\\[-0.35in]
\\[-0.1in]
\spc{\small Stan 2.18.0 \ {\footnotesize (June 2018)} \hfill \url{http://mc-stan.org}}
\hfill
...
\mypart{}{Reparameterizations}

\sld{Why Reparameterize?}
%
\begin{itemize}
\item Statistical reasons
\begin{subitemize}
\item more natural priors (e.g, Beta distribution)
\item different scales (e.g., lognormal, inverse gamma)
\item links for generalized linear models (e.g., inverse logit)
\end{subitemize}
\item Computational reasons
\begin{subitemize}
\item standardize scales
\item reduce dependence among variables
\item reduce curvature
\begin{subsubitemize}
\item often related to interaction of scale and dependence
\end{subsubitemize}
\end{subitemize}
\end{itemize}

\sld{Beta Reparameterization}
%
\begin{itemize}
\item $\mathsf{Beta}(\alpha, \beta)$ for $\alpha, \beta > 1$ is
  standard
\begin{subitemize}
\item $\alpha$ and $\beta$ are ``prior'' success and failure counts plus one
\item difficult to specify mean and variance in prior
\end{subitemize}
\item Reparameterize with $\theta = \alpha / \beta$ and $\kappa =
  \alpha + \beta$.
\begin{subitemize}
\item $\theta$ is mean and $\kappa$ is prior count plus two
\item $\kappa$ is inversely related to variance
\end{subitemize}
...
\sld{Log-Odds Reparameterization \hfill \large (1/3)}
%
\begin{itemize}
...
\sld{Log-Odds Reparameterization \hfill \large (2/3)}
%
\begin{itemize}
...
\sld{Log-Odds Reparameterization \hfill \large (3/3)}
%
...
\sld{Jacobian Calculation, 1D \hfill \large (1/2)}
%
\begin{itemize}
...
\sld{Funnel-Shaped Posteriors \hfill \large (1/2)}
%
\vspace*{-6pt}
\begin{itemize}
\item Arise in hierarchical model with no data (Neal 2003)
\begin{subitemize}
\item $\log \sigma \sim \mathsf{Normal}(0, 1.5)$
  \hfill {\small [hierarchical scale]}
\item $\beta_n \sim \mathsf{Normal}(0, \sigma)$ \mbox{ for } $n \in
  1:9$
 \hfill {\small [low-level coefficients]}
\end{subitemize}
\vspace*{-12pt}
\begin{center}
  \includegraphics[height=1.05in]{img/funnel.png}
  \includegraphics[height = 1.05in]{img/funnel-fit.png}
\end{center}
\vspace*{-12pt}
{\footnotesize  $\beta_1$ coefficient ($x$-axis) vs.\ $\log
   \sigma$ ($y$-axis);  {\slshape left}) density plot (log scale);  {\slshape right})
  4000 independent draws}
\\
{\tiny \mbox{ } \hfill Neal, R. M. 2003. Slice sampling. {\slshape Annals of
    Statistics}, {\bfseries 31}(3):705--767}
\end{itemize}

\sld{Funnel-Shaped Posteriors \hfill \large (2/2)}
%
\begin{itemize}
\item Very \myemph{challenging for sampling}
\item Need large step size to explore mouth of funnel
\item Need small step size to explore neck of funnel
\item Even small step sizes lead to divergences
\begin{subitemize}
\item numerical failure of Hamiltonian dynamics simulation to conserve the Hamiltonian
\end{subitemize}
\item Betancourt and Girolami (2015) analyzed for Hamiltonian Monte Carlo
\vfill
{\tiny Betancourt and Girolami. 2015. Hamiltonian Monte Carlo for
  hierarchical models. \\[-3pt] In {\slshape Current Trends in Bayesian
    Methodology with Applications}. CRC}
\end{itemize}

\sld{Non-Centered Parameterization}
%
\begin{itemize}
\item The non-centered parameterization of the funnel is
\begin{eqnarray*}
\log \sigma & \sim & \mathsf{Normal}(0, 1.5)
\\[4pt]
\beta^{\mathrm{std}}_n & \sim & \mathsf{Normal}(0, 1)
\\[4pt]
\beta_n & = & \sigma \times \beta^{\mathrm{std}}_n
\end{eqnarray*}
\item Removes dependency of $\beta$ on $\sigma$ in prior
\item \footnotesize
Called it ``Matt trick'' (after Matt Hoffman) before realizing it was
  well-known
\end{itemize}
...
%
\begin{itemize}
\item Use the $\beta$ in an intercept-only binomial logistic
  regression,
\[
y_j \sim \mathsf{Binomial}(N, \mathrm{logit}^{-1}(\beta_j))
\]
\vspace*{-24pt}
\begin{subitemize}
\item i.e., $y_j \in 0:N$ is number of successes in $K$ trials
\item $\beta_n$ is log odds of success for group $j$
\end{subitemize}
\item More data lessens dependency between $\beta$ and $\sigma$
\item With informative enough data, centered parameterization is
  better
\begin{subitemize}
\item not size of data, but how much it constrains posterior
\end{subitemize}
\end{itemize}


\sld{Non-Centered + Data = Funnel}
%
\begin{itemize}
\item With more data, centered approaches independent normal
\item Non-centering ($\beta^{\mathrm{std}} = \beta / \sigma$) produces a funnel
\item Centered parameterizations dominate with lots of data
\end{itemize}
\begin{center}
\includegraphics[width=0.8\textwidth]{img/non-centering-funnel.pdf}
\end{center}

\sld{Funnel + Data in the Wild}
%
\\ \spc
% \includegraphics[width=0.8\textwidth]{img/funnel-fit-data.png}
\includegraphics[width=0.8\textwidth]{img/funnel-fit.png}


\mypart{}{Mixture Models}

\sld{Simple Normal Mixture \hfill \large (1/3)}
%
\begin{itemize}
\item Assume a population is a mixture of two subpopulations
\begin{subitemize}
\item subpopulation one is distributed $\mathsf{Normal}(\mu_1,
  \sigma_1)$
\item subpopulation two is distributed $\mathsf{Normal}(\mu_2,
  \sigma_2)$
\item subpopulation one makes up $\lambda \in (0, 1)$ of the population
\end{subitemize}
\item Model a draw $y_n$ from the population as follows
\begin{subitemize}
\item draw subpopulation indicator $z_n \sim \mathsf{Bernoulli}(\lambda)$
\item if $z_n = 1$ draw $y_n \sim \mathsf{Normal}(\mu_1, \sigma_1)$,
  \\
else draw $y_n \sim \mathsf{Normal}(\mu_2, \sigma_2)$
\end{subitemize}
...
\sld{Normal Mixture \hfill \large (2 / 3)}
%
\begin{itemize}
...
\sld{Normal Mixture \hfill \large (3/3)}
%
\begin{itemize}
...
\end{array}
\]
\item In Stan code, use \code{log\_sum\_exp}
\begin{stancode}
for (n in 1:N) {
  real lp1 = log(lambda)
...
\sld{Normal Mixture \hfill \large (bonus 1/2)}
%
\begin{itemize}
...
for (n in 1:N)
  target += log_mix(lambda,
                    ...
                    normal_lpdf(y[n] | mu[2], sigma[2]));
\end{stancode}
\item \code{lambda} is a probability between zero and one
\item second two arguments are log-scale densities or masses
\end{itemize}

\sld{Normal Mixture \hfill \large (bonus 2/2)}
%
\begin{itemize}
...
\sld{Zero-Inflated Poisson}
%
\begin{itemize}
\item Poisson is count of phenomenon parameterized by a single
  parameter $\lambda$
\begin{subitemize}
\item $y_n \sim \mathsf{Poisson}(\lambda)$
\item $\lambda$ is mean
\item $\lambda$ is also variance ($\sqrt{\lambda}$ is scale)
\end{subitemize}
\item Sometimes more zero values than you'd expect from Poisson itself
\item Simple branching generative process
\begin{subitemize}
\item $z_n \sim \mathsf{Bernoulli}(\theta)$
\item if $z_n == 1$, then $y_n = 0$
\item else if $z_n == 0$, $y_n \sim \mathsf{Poisson}(\lambda)$
\end{subitemize}
\end{itemize}

\sld{Zero-Inflated Poisson in Stan}
%
\begin{itemize}
\item Marginalize out as usual---addition becomes log-sum-exp
\[
p(y_n \mid \lambda, \theta)
= \lambda \times \mathrm{I}[y_n = 0]
+ (1 - \lambda) \times \mathsf{Poisson}(y_n \mid \lambda)
\]
\item Coding the likelihood in Stan {\small (branch to simplify non-zero)}
{\small
\begin{stancode}
real z_zero_lp = log(theta);
real z_any_lp = log1m(theta);
for (n in 1:N) {
  real poisson_lp = poisson_lpmf(y[n] | lambda);
  if (y[n] == 0)
    target += log_sum_exp(z_zero_lp, z_any_lp + poisson_lp);
  else
    target += z_any_lp + poisson_lp;
}
\end{stancode}
}
\end{itemize}
...
%
\begin{itemize}
\item Intuitions formed in low dimensions \myemph{do not
    generalize}
...
\item the average member of a multivariate normal is the mean
...
\begin{subitemize}
%
\item \myemph{MCMC draws} from posterior $p(\theta | y)$
\[
\theta^{(1)}, \ldots, \theta^{(M)}
\]
...
%
\begin{subitemize}
\item Posterior \myemph{event probabilities}
\[
\mbox{Pr}[\mbox{cond}(\theta) \mid y]
\ = \
\int \mathrm{I}[\mbox{cond}(\theta) \mid y] \, \mathrm{d}\theta
\ \approx \
\frac{1}{M} \sum_{m=1}^M \mathrm{I}[\mbox{cond}(\theta^{(m)})]
\]
%
\item Posterior \myemph{predictions}
\[
p(\tilde{y} | y)
 =
\mathbb{E}[p(\tilde{y}) | y]
...
\ \approx \
\frac{1}{M} \sum_{m=1}^M p\left( \tilde{y} | \theta^{(m)} \right)
\]
%
...
\mypart{}{Philosophy\\[12pt]\spc of Probability}
...
sex, income, education level, religious affiliation,
...
% duplicate from top-level intro
\sld{What is Stan?}
%
\begin{itemize}
\item Stan is an \myemph{imperative} probabilistic programming language
  \begin{subitemize}
  \item  cf., BUGS: declarative; \ Church: functional
  \end{subitemize}
\item Stan \myemph{program} defines a probability model
  \begin{subitemize}
  \item declares data and (constrained) parameter variables
...
\sld{Books about Stan \hfill \large (1/2)}
%
\begin{itemize}
...
\item Johnson, A.A., M. Q. Ott, M. Dogucu. 2021. {\slshape Bayes
    Rules! An Introduction to Applied Bayesian Modeling.} CRC Press.
\item McElreath (2020) {\slshape Statistical Rethinking: A Bayesian course 
    with R and Stan}. CRC. Second Edition.
\item Gelman and Hill (2020) {\slshape Regression and Other Stories}. Cambridge.
\item Holmes, E. E., M. D. Scheuerell, and E. J. Ward. 2019. {\slshape
    Applied Time Series Analysis for Fisheries and Environmental
    Sciences}. NOAA Fisheries, Northwest Fisheries Science Center.
...
\sld{Books about Stan \hfill \large (2/2)}
%
\begin{itemize}
...
\item McElreath (2016) {\slshape Statistical Rethinking: A Bayesian course 
    with R and Stan}. CRC. First Edition.
...
\begin{center}
\hspace*{12pt}
% \includegraphics[width=0.9\textwidth]{img/adaptation-windows.pdf}
\end{center}
\vspace*{-12pt}
\begin{subitemize}
\item (I) initial fast interval to find typical set
\\
(step size,   default 75 iterations
\item (II) expanding memoryless windows to estimate metric
\\
(step size \& metric, initial 25 its)
\item (III) final fast interval (step size, default 50 its)
\end{subitemize}
...
\item Coda-like \myemph{posterior summary}
  ...
  \end{itemize}
\item Model comparison with leave-one-out cross-validation (loo) [in
  sample only]
\begin{subitemize}
...
\item take gradient step to minimize KL-divergence
\end{subitemize}
\vfill
...
\begin{subsubitemize}
\item hierarchical variance controls lower-level parameters
\end{subsubitemize}
\end{subitemize}
