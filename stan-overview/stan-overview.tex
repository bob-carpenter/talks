...
\mbox{ }
\vspace*{0.5in}
\hfill \includegraphics[width=0.8\textwidth]{img/volterra-orbit.jpg}
\end{minipage}
\vfill
\hfill
{\tiny
...
\]
\vspace*{-4pt}
{\small where $\hat{z}_n$ is \myemph{solution} at $t_n$ to diff eqs for initial
$z^{\mathrm{init}}$}
\\[-3pt]
\vfill
...
\item initial time (\code{0}), solution times (\code{ts})
\item parameters (\code{theta}), data arrays (unused)
\item tolerances (\code{1e-6}, \code{1-e5}), iterations (\code{1e3})
\end{subitemize}
\end{itemize}
{\small
\begin{stancode}
...
\sld{\Large Lotka-Volterra Fit (vs. training data)}
%
\\
\spc \hfill \includegraphics[width=0.8\textwidth]{img/lotka-volterra-predict.png}
...
\item Imagine a bin filled with balls represneting the way the world might be
