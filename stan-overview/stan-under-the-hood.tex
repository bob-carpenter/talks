...
\item Stan is an \myemph{imperative probabilistic programming language}
  \begin{subitemize}
  \item  cf., BUGS: declarative; \ Church: functional; \ PyMC: hybrid
  \end{subitemize}
...
\item Stan \myemph{inference}: fits model to data \& makes predictions
  ...
  \item Laplace approximation and VB (ADVI, Pathfinder) for approximate Bayes
  \item MLE and bootstrap for confidence intervals (L-BFGS)
  \end{subitemize}
\end{itemize}
...
\item {\slshape Problem}:  Autodiff requires functions templated on
  all arguments.
\item {\slshape Solution}: Our own C++ density and special functions library, Eigen linear algebra, Boost foundation (bulk of our dev cost)
%
\vspace*{8pt}
...
%
\vspace*{8pt}
\item {\slshape Problem}:  Pure directed graphical language inflexible (cf. BUGS).
...
\vspace*{8pt}
\item {\slshape Problem}:  Non-statistician or non-programmer users.
\item {\slshape Solution}: High-level interfaces (brms, Prophet, rstanarm, 100+ dependent packages).
\vspace*{8pt}
\item {\slshape Problem}:  Restrictive licensing limits use.
\item {\slshape Solution}: Code and doc open source (BSD, CC-BY)
\end{itemize}

\sld{\Large Stan Estimates Expectations \hfill \large (1/2)}
%
\begin{itemize}
%
\item Uses \myemph{MCMC draws} from posterior $p(\theta \mid y)$:
\[
...
\end{itemize}
\begin{center}
  \includegraphics[height=0.25\textwidth]{img/lynx.jpeg}
  \quad
  \includegraphics[height=0.25\textwidth]{img/hare.jpeg}
\end{center}
\begin{itemize}
...
\end{itemize}
\begin{center}
  \includegraphics[width=0.6\textwidth]{img/hudons-bay-data.png}
\end{center}

\sld{Plotted pelt data}
\begin{center}
\includegraphics[width=0.35\textwidth]{img/lynx-hares-1.png}\qquad\includegraphics[width=0.35\textwidth]{img/lynx-hares-2.png}
\end{center}
\begin{itemize}
...
\item pelt data assumed proportional to population; \myemph{model predicts pelts}
\end{itemize}
...
\end{subitemize}
\begin{center}
\includegraphics[width=0.5\textwidth]{img/lotka-volterra-posterior.pdf}
\end{center}

\sld{100 predictive draws}
\begin{center}
\includegraphics[width=0.6\textwidth]{img/lotka-volterra-posterior-time.png}
\end{center}
...
\sld{Summary}
\begin{itemize}
\item Start with mechanistic forward model of dynamics (Lotka 1926)
\begin{subitemize}
\item $\hat{y} = f(\theta)$
\end{subitemize}
...
\begin{itemize}
\item \small We like to reproduce the analyses in papers to illustrate Stan.
\item Brian Ward (Flatiron, core Stan developer) and I reproduced David Barmherzig's
  paper on [{\slshape Star Trek} mumbo jumbo] to illustrate Stan's complex number and fast Fourier transform (FFT) capabilities
\item \footnotesize GitHub: \myemph{\texttt{bob-carpenter/CDI}}
\end{itemize}
\begin{center}
  \includegraphics[width=0.25\textwidth]{img/barmherzig-cite.png}
\end{center}
...
\vspace*{-8pt}
\begin{center}
  \includegraphics[width=0.4\textwidth]{img/fft/truth-ref.png}
\end{center}
...
\item \small Step 2: observe diffracted \myemph{photon counts} $\tilde{Y}_{i,j}$
  ...
  \end{subitemize}
  \item Known to follow a \myemph{Poisson process} according to [insert physics theory].
\end{itemize}
\vspace*{-8pt}
\begin{center}
  \includegraphics[width=0.55\textwidth]{img/fft/observed.png}
\end{center}
...
\item For the \myemph{$256 \times 256$} pixel reconstructions in David's paper,
  \begin{subitemize}
  \item optimization solves inverse problem in \myemph{2 minutes} ($5 \times$ times faster than original MATLAB)
...
\end{itemize}
\begin{center}
  \includegraphics[width=0.3\textwidth]{img/fft/recover-256-1.png}
  \includegraphics[width=0.3\textwidth]{img/fft/recover-256-10.png}
\end{center}
\vspace*{-12pt}
\begin{itemize}
...
\begin{subitemize}
\item \myemph{Primitive}: {\tt\small int}, \ {\tt\small real}, \ {\tt\small complex}
\item \myemph{Matrix}: {\tt\small matrix[M,N]}, \ {\tt\small vector[M]}, \ {\tt\small row\_vector[N]}
  {\tt\small complex\_matrix[M,N]}, \\ {\tt\small complex\_vector[M]}, \ {\tt\small complex\_row\_vector[N]}
\item \myemph{Bounded}: primitive/matrix, w.\
  {\tt\small <lower=L>}, \ {\tt\small <upper=U>}, \ {\tt\small <lower=L,upper=U>}
\item \myemph{Constrained Vectors}: {\tt\small simplex[K]}, \ {\tt\small
    ordered[N]}, \ {\tt\small sum\_to\_zero\_vector[K]}
...
\item \myemph{Constrained Matrices}: {\tt\small cov\_matrix[K]}, \ {\tt\small
  ...
  {\tt\small cholesky\_factor\_corr[K]}, \\ {\tt\small left\_stochastic\_matrix[K]},
  {\tt\small right\_stochastic\_matrix[K]}
...
\begin{subitemize}
\item e.g., \Verb|complex_matrix *= matrix;|
\item e.g., \Verb|vector *= real;|
\end{subitemize}
\end{itemize}
...
\end{itemize}
\vspace*{12pt}
\spc{\LARGE\myemph{Block of Statements}}
%
\begin{itemize}
